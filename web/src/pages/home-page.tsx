		...
		<div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 pt-8 pb-20">
			...
			<section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
				<MetricCard
					...
					value={stats.isSuccess ? total : '—'}
					detail="registros disponibles"
					icon={IconDatabase}
					accentClass="bg-cat-backend"
				/>
				<MetricCard
					label="Añadidos esta semana"
					value={stats.isSuccess ? stats.data.addedThisWeek : '—'}
				...
				<MetricCard
					...
					value={stats.isSuccess ? activeCategories : '—'}
					detail="de 8 categorías posibles"
					icon={IconSparkles}
					accentClass="bg-cat-datos-ia"
				/>
			</section>
			...
			<section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.9fr)]">
						...
						{stats.isLoading
							...
							: categoryOrder.map(category => {
									...
									return (
										<div
											key={category}
											className="grid grid-cols-[145px_minmax(0,1fr)_42px] items-center gap-3 text-sm"
										>
											<span className="flex items-center gap-2 text-muted-foreground">
												<span className={cn('size-1.5 rounded-full', categoryColor[category])} />
												{category}
											</span>
											<div className="h-1.5 overflow-hidden rounded-sm bg-muted">
												<div
													className={cn('h-full rounded-sm', categoryColor[category])}
				...
				<div className="rounded-md border bg-card p-5">
					...
					<div className="mt-4 grid gap-2">
						<Link
							to="/analizar"
							className="flex items-center justify-between rounded-md bg-primary px-4 py-3 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90"
						>
						...
						>
							<span className="flex items-center gap-2">
								<IconMap2 className="size-4 text-muted-foreground" />
								Explorar el mapa
							</span>
							<IconArrowRight className="size-4 text-muted-foreground" />
						</Link>
