...
import { batchUploadResponseSchema, type BatchUploadResponse } from '@/shared/api/contracts';
...
export async function uploadBatch(file: File): Promise<BatchUploadResponse> {
	...
	const { data } = await apiClient.post('/contents/batch', formData, { timeout: 300_000 });
