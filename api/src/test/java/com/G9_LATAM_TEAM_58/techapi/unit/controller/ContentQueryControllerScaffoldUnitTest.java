package com.G9_LATAM_TEAM_58.techapi.unit.controller;

import com.G9_LATAM_TEAM_58.techapi.core.controller.ContentQueryController;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.webmvc.test.autoconfigure.WebMvcTest;
import org.springframework.http.HttpStatus;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.test.web.servlet.MvcResult;

import static com.G9_LATAM_TEAM_58.techapi.unit.controller.SharedTestAssertions.assertApiError;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

@WebMvcTest(ContentQueryController.class)
class ContentQueryControllerScaffoldUnitTest {

    @Autowired
    private MockMvc mvc;

    @Test
    void testListScaffold503() throws Exception {
        // No IContentQueryService mock -> the Optional dependency is empty and
        // the controller throws InferenceUnavailableException -> 503.
        MvcResult result = mvc.perform(get("/contents"))
                .andExpect(status().isServiceUnavailable())
                .andReturn();

        assertApiError(result, HttpStatus.SERVICE_UNAVAILABLE, "INTERNAL_ERROR");
    }

    @Test
    void testDetailScaffold503() throws Exception {
        // No IContentQueryService mock -> the Optional dependency is empty and
        // getContentById throws InferenceUnavailableException -> 503.
        MvcResult result = mvc.perform(get("/contents/so-32472760"))
                .andExpect(status().isServiceUnavailable())
                .andReturn();

        assertApiError(result, HttpStatus.SERVICE_UNAVAILABLE, "INTERNAL_ERROR");
    }
}
